...
// array destination[3] filled with absolute coordinates is fed into this. tobben 20. may 2015
  void calculate_line_lengths(float cartesian[3], float line_lengths[4])
  {
    ...
    line_lengths[D_AXIS] =                      cartesian[Z_AXIS];

    /*
    line_lengths[D_AXIS] =                      cartesian[Z_AXIS];
                              sqrt(sq(          cartesian[X_AXIS])
                              + sq(             cartesian[Y_AXIS])
    ...
    /*
    // following was added by Chris Dagher for alternate kinematic system derived from hangprinter
    carriage_A_f = cartesian[Z_AXIS]/sin(60);  // distance from A carriage origin on diagonal beam
    carriage_A_x = anchor_A_x;  // A carriage X coordinate
    carriage_A_y = anchor_A_y + sqrt(sq(cartesian[Z_AXIS]) + sq(carriage_A_f));  // calculate A carriage Y coordinate

    carriage_B_f = carriage_A_f;//cartesian[Z_AXIS]/sin(60);  // distance from B carriage origin on diagonal beam
    carriage_B_x = anchor_B_x - sqrt(sq(carriage_A_y) + sq(carriage_B_y));  // calculate B carriage X coordinate
    carriage_B_y = anchor_B_y - sin(60)/carriage_A_y;  // calculate B carriage Y coordinate

    carriage_C_f = carriage_A_f;//cartesian[Z_AXIS]/sin(60);  // distance from B carriage ogigin on diagonal beam
    carriage_C_x = carriage_B_x;  // calculate C axis X coordinate
    carriage_C_y = -carriage_B_y;  // calculate C carriage Y coordinate

    line_lengths[A_AXIS] = sqrt(sq(carriage_A_y - cartesian[Y_AXIS])
                              + sq(carriage_A_x - cartesian[X_AXIS]));
    line_lengths[B_AXIS] = sqrt(sq(carriage_A_y - cartesian[Y_AXIS])
                              + sq(carriage_A_x - cartesian[X_AXIS]));
    line_lengths[C_AXIS] = sqrt(sq(carriage_A_y - cartesian[Y_AXIS])
                              + sq(carriage_A_x - cartesian[X_AXIS]));
    line_lengths[D_AXIS] = sqrt(sq(               cartesian[X_AXIS])
                              + sq(               cartesian[Y_AXIS])
                              + sq(anchor_D_z -   cartesian[Z_AXIS]));
    //line_lengths[F_AXIS] = F_axis_origin - carriage_A_f;
    */
  }
